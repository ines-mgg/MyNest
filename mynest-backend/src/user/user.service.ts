...
import { ADMIN, USER } from 'src/constants/roles';
...
export class UserService {
  ...
  async getProfil(id: userDto['id']) {
    const userProfil = await this.prisma.user.findUnique({
      where: {
        id,
      },
      select: {
  ...
  async search(
    query: string,
    searcherId: userDto['id'],
    searcherRole: userDto['role'],
  ) {
    const users = await this.prisma.user.findMany({
      where: {
        id: {
        ...
        role: searcherRole === USER ? USER : { in: [ADMIN, USER] },
        OR: [
          {
            username: {
              contains: query,
  ...
  async uploadProfilPicture(userId: userDto['id'], filename: string) {
    const user = await this.prisma.user.update({
      where: {
        id: userId,
      },
